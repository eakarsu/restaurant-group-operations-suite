const baseUrl = process.env.SMOKE_BASE_URL || 'http://127.0.0.1:5320';
...
const adminCookie = await login('admin@restaurant-group-operations.local', 'admin123');
const managerCookie = await login('manager@restaurant-group-operations.local', 'manager123');
const analystCookie = await login('analyst@restaurant-group-operations.local', 'analyst123');
